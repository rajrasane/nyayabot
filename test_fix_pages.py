import json
import sqlite3
from backend.main import get_db, _find_best_match_in_page
    ...
    with get_db() as conn:
        ...
        for row in rows:
            ...
            for d in extraction.get('directives', []):
                ...
                # Check all pages to find the actual highest overlap
                for p in range(1, page_count + 1):
                    page_text = get_page_raw_text(raw_text, p)
                    candidates = _find_best_match_in_page(d['text'], page_text)
                    if candidates:
                        # calculate overlap score of the best candidate
                        # simple heuristic: if it returned candidates, it means overlap >= 2
                        # Let's count actual overlap words for the first candidate
